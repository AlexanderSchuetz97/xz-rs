...
fn run_test(seed: [usize; 32]) {
    ...
    loop {
        ...
        match dec.decode(&sl[..seed[count].min(sl.len())], &mut sl2[..seed[count2].min(sl2len)]).expect("failed to decode") {
...
fn cb_t1() {
    ...
    run_test([3usize,7,4,2,1,2,8,1,5,7,1, 11, 1, 3,4,2,1,9,2,1,3,2,4,6,3,3,1,8,7,3,1,2])
}
...
fn cb_t2() {
    ...
    run_test([3usize,7,4,2,1,2,8,1,5,7,1, 11, 65, 3,4,2,1,9,2,1,3,2,4,6,3,3,1,8,7,3,1,2])
}